...
├── test_sqlite.sh          # SQLite 库测试（41 个测试用例）
├── test_cli.sh             # CLI 命令测试（34 个测试用例）
├── test_edge_cases.sh      # 边界条件测试（36 个测试用例）
├── test_hooks.sh           # Hooks 功能测试（24 个测试用例）
├── run_tests.sh            # 测试运行器
└── TEST-REPORT.md          # 本文档
```
